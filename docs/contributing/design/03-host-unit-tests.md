...
`WRITE_REGISTER_ULONG`, `KeStallExecutionProcessor`, `InterlockedIncrement`,
and the three spin-lock entry points) at hooks a test provides, declared in
`src/xhci_compat.h` under `XHCI_HOST_TEST` alongside the type block.
...
| the seven registered no-op callbacks (`PollController`, `PollEndpoint`, `RebalanceEndpoint`, `TakePortControl`, `StartSendOnePacket`, `EndSendOnePacket`, `PassThru`) | full bracket each | every path - none of them touches a register or changes state, and `PassThru` alone has a body, the snapshot instrument's | `test_registered_unexercised_callbacks`: each slot is non-NULL in the packet, and each is called and asserted to have touched nothing and moved no Flags. The row exists because the slots must be filled - usbport calls through them without asking whether the miniport wants them - and because `PassThru` is the one of the seven that both shipping usbport builds really do call, which is what the vendor IOCTL channel `docs/contributing/passthru-snapshot-instrument.md` describes rides on. A NULL in any of these slots is a jump through a null pointer at DISPATCH with no diagnostic on either guest |
...
| Context builders | `test_ctx` | Slot/Endpoint context DWORDs for known inputs, at both context strides; Input Control Context A/D flags for Address Device / Configure Endpoint / Evaluate Context shapes |
...
| SET_ADDRESS interception | `test_init` (`test_slot_transfers`, `test_slot_set_address_refusals`) | The EP0 setup-packet classifier flags exactly bmRequestType 0x00 / bRequest 0x05 and nothing else, nothing is placed on the ring, the transfer completes exactly once when the Address Device answers; address-map lookups stay correct when usbport and xHC addresses coincide |
...
- One runner per pure-core module, plain C89, no framework: a `CHECK(cond)` /
  ...
  | `test_init` | `src/xhci_init.c` + `src/xhci_evt.c` + `src/xhci_cmd.c` + `src/xhci_rh.c` + `src/xhci_pci.c` + `src/xhci_dispatch.c` + `src/xhci_slot.c` + `src/xhci_probe.c`, plus the nine pure-core files they pull in (17 in all; `test/run-host-tests.cmd` is the list) - the init sequence, the run/port-power/quiesce paths, the interrupt path, the command engine, the root-hub callback family, the slot/endpoint lifecycle, and the registered callback surface, with their access order, against a synthetic controller. `DriverEntry` is excluded under `XHCI_HOST_TEST`, so the file contributes its wrappers and the packet without its DDK dependencies. It also links `src/xhci_port.c`, because the port-power step and the root-hub callbacks compose their PORTSC values there rather than by hand |
  ...
  | `test_log` | `src/xhci_log.c` - task 11-V.7's optional log ring, on its own because it is pure by construction and has to stay that way: the ring, its wrap and which byte it loses, the record cap, the FIFO chunked drain and the flush verdict are all here, and the file I/O is not. Two accounting nets in task 7b-A.1.0's shape (`Appends + Suppressed == calls made`, and bytes conserved across drains, drops and what the ring still holds), each with a never-reset twin. The half it does not cover is named rather than implied - the `KeGetCurrentIrql` guard and the two registry reads live in `src/xhci_dispatch.c` and are driven through `test_init`'s stand-ins (the ring-0 file sink and its `ZwCreateFile`/`ZwWriteFile` imports were retired in task 13-L.2; only comments recording the removal remain) |
...
- Build+run is one `cl.exe /W3 /WX /Za` command per runner on the Windows
  host, using the same MSVC 6.0 the driver build uses. `/Za` is not
  decoration: it is the C89 dialect gate (no `//`, no mid-block declarations),
  and it is the only one - the DDK build compiles with `/Ze` (every `cl` line
  in a build log says so), so a dialect error in a file this suite does not
  compile (`xhci_dbg.c`) is caught by nothing. `/WX` makes every warning a
  failure, as the driver build does.
...
command engine: capturing the start epoch under the controller lock (`xhciControllerLock`, through `XhciControllerLockAcquire`) rather than
