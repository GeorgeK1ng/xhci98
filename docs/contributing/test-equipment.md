...
| 2 | Windows 98 SE CD (not a USB device; it is what made row 1's cheap branch runnable) | 13-E.1 | That it is the SE CD and its USB components are on it | Confirmed by the project owner, and verified by hash on the host rather than at the bench: the disc's own `layout.inf` (out of `PRECOPY1.CAB`) lists the full USB stack, and its `usbd.sys` (18,912 bytes) hashed identically to the Win98 SE row of the manifest the package then carried (`scripts\package\usbd-sources.expected`), which also proved the disc was the media the package's `usbd98.sys` came from. The manifest and the packaged copy were retired in 1.0.0.1, when the INF began having the OS supply the file; the reading stands as taken. The CD contents were copied onto the E460's own disk so stage E5 installs from hard disk; `run-13e.md` P4 has the recipe for checking a different disc |
